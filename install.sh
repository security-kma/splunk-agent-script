...
if [  -n "$(uname -a | grep Ubuntu)" ]; then
   sudo apt install auditd sshpass -y
   
else
    yum install auditd sshpass -y
fi  
...
echo "[*]Finished install auditd"

echo "[*]Installing universal splunk forwarder"

server_IP="192.168.41.135"
server_port="9997"
username="misa"
password="12345678@"
echo "[*]Dowloading universal splunk forwarder"
sshpass -p "12345678@" scp misa@server_IP:/home/misa/splunk/splunkforwarder-8.2.2.1-ae6821b7c64b-linux-2.6-amd64.deb .

if [  -n "$(uname -a | grep Ubuntu)" ]; then
	echo "[*]For Ubuntu ..."
	#wget -O splunkforwarder-8.2.2.1-ae6821b7c64b-linux-2.6-amd64.deb 'https://d7wz6hmoaavd0.cloudfront.net/products/universalforwarder/releases/8.2.2.1/linux/splunkforwarder-8.2.2.1-ae6821b7c64b-linux-2.6-amd64.deb' &&
...
sudo /opt/splunkforwarder/bin/splunk add forward-server {server_IP}:{server_port} -auth {username}:{password} &&
sudo /opt/splunkforwarder/bin/splunk add forward-server {server_IP}:{server_port} -auth {username}:{password} &&
sudo /opt/splunkforwarder/bin/splunk add monitor /var/log &&
sudo /opt/splunkforwarder/bin/splunk restart &&
wait 30
	
echo "[*]Finished install splunk forwarder"
